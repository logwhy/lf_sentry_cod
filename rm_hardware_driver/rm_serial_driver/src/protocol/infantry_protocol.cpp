...
void ProtocolInfantry::send(const rm_interfaces::msg::GimbalCmd &data) { 
  ...
  packet.loadData<float>(static_cast<float>(data.tp), 14);
  packet.loadData<float>(static_cast<float>(data.ty), 18);
