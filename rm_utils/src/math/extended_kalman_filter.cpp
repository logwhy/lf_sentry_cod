...
Eigen::MatrixXd ExtendedKalmanFilter::update(const Eigen::VectorXd &z) noexcept {
  ...
  // NIS (Normalized Innovation Squared) for divergence detection
  last_nis = (residual.transpose() * S.inverse() * residual)(0, 0);
  recent_nis_failures.push_back(last_nis > nis_threshold ? 1 : 0);
  if (recent_nis_failures.size() > nis_window_size) recent_nis_failures.pop_front();
