...
public:
  ...
  size_t nis_window_size = 50;
  double nis_threshold = 9.49;  // chi-sq df=4, 95% confidence

  bool isDiverged() const {
    if (recent_nis_failures.size() < nis_window_size) return false;
    int fail_count = std::accumulate(recent_nis_failures.begin(), recent_nis_failures.end(), 0);
    return fail_count >= static_cast<int>(0.4 * nis_window_size);
  }
