...
void Tracker::init(const Armors::SharedPtr &armors_msg) noexcept {
  ...
  FYT_INFO("armor_solver", "Init armor: %s", tracked_armor.number.c_str());   
...
void Tracker::update(const Armors::SharedPtr &armors_msg) noexcept {
  ...
  if (!armors_msg->armors.empty()) {
    ...
    if (min_position_diff < max_match_distance_ && yaw_diff < max_match_yaw_diff_) {
      ...
      // Check EKF divergence via NIS
      if (ekf.isDiverged()) {
        FYT_WARN("armor_solver", "EKF diverged (NIS failure rate > 40%%), resetting tracker");
        tracker_state = LOST;
        return;
      }
      // if (0 < distance && distance < 0.2) {
      //   // 直接使用观察到的位置作为目标状态
      //   target_state(0) = p.x;  // xc
      //   target_state(1) = 0;    // vxc
      //   target_state(2) = p.y;  // yc
      //   target_state(3) = 0;    // vyc
      //   target_state(4) = p.z;  // za
      //   target_state(5) = 0;    // vza
      //   target_state(6) = measured_yaw;  // yaw
      //   target_state(7) = 0;    // v_yaw
      //   // radius 保持不变或设置为一个较小的值
      //   target_state(8) = 0.15;  // r
      //   FYT_DEBUG("armor_solver", "Distance < 0.2, using observation directly");
      // } else {
        // 正常情况使用卡尔曼滤波更新
